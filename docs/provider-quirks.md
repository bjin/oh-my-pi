...
- **Provider Protocol & Request Envelope**:
  ...
  - **System Instructions**: Antigravity tags system instructions with `role: "user"`. No identity prompt is injected. When Cloud Code answers a top-level `systemInstruction` with a synthetic 429 (`RESOURCE_EXHAUSTED`, generic "Resource has been exhausted (e.g. check quota)." message, no `details`; `isAntigravitySynthetic429`), `streamGoogleGeminiCli` skips `fetchWithRetry` replays, folds the system prompt into the first user turn, and retries the same endpoint once; the wire model id is recorded in `AntigravityProviderSessionState.foldedSystemInstructionModels` so `buildRequest` folds up front for the rest of the conversation.
  - **Request Envelope & Session State**: Antigravity wraps requests in `buildAntigravityRequestEnvelope`: `project` (projectId), `requestId` (`agent/<agentId>/<ts>/<trajectoryId>/<step>`), `userAgent` (`antigravity`), and `labels` (`last_step_index`, `model_enum`, `trajectory_id`, `used_claude`, `used_claude_conservative`, `last_execution_id`). `requestType` is omitted, as in the official client: `"agent"` arms Cloud Code's systemInstruction filter, which answers quota-healthy accounts with the synthetic 429 above. State maintains monotonic `stepIndex`, persistent `agentId`, `trajectoryId`, and signed-decimal `sessionId` (`deriveAntigravitySessionId`).
...
- **System Instruction & Request Envelope**: Antigravity tags `systemInstruction` with `role: "user"` and sends the caller's prompts unmodified, folding them into the first user turn only after a synthetic 429 (see Cloud Code Assist above). `buildAntigravityRequestEnvelope` injects structured `requestId` (`agent/<id>/<ts>/<trajectoryId>/<step>`), `sessionId`, and `labels` (`model_enum`, `trajectory_id`, `last_step_index`, `last_execution_id`, `used_claude*`) using `getAntigravityModelWireProfile`; `buildRequest` adds `userAgent: "antigravity"`. No Antigravity envelope (chat, image generation, Gemini web search) sends `requestType`.
